...
# ResNet Classification script that performs cross-validation

# Path to the CSV file containing image paths and labels
csv_path = '/home/sruyss8/Ultimate_coordinates.csv'
df = pd.read_csv(csv_path)

# Optional: Shuffle the DataFrame (commented out)
# df = df.sample(frac=1, random_state=317).reset_index(drop=True)
...
# List of column names for coordinates in the DataFrame
coordinate_cols = [
    'TEA_X1', 'TEA_Y1', 'TEA_Z1', 'TEA_X2', 'TEA_Y2', 'TEA_Z2',
    'PCA_X1', 'PCA_Y1', 'PCA_Z1', 'PCA_X2', 'PCA_Y2', 'PCA_Z2'
]

def load_data(df, coordinate_cols):
    """
    Load image paths and corresponding coordinate labels from the DataFrame.
    ...
    Returns:
        Tuple: List of image paths and tensor of coordinate labels.
    """
...
# Create a KFold object to split the dataset into training and validation sets
...
for fold, (train_index, val_index) in enumerate(kf.split(image_paths)):
    ...
    # Retrieve training and validation data for the current fold
    ...
    # Define model parameters and file paths for saving
    model_name = f'Res10fold{fold}'
    learning_rate = 0.03
    weight_decay = 0
    sched_step_size = 100
    sched_gamma = 0.1
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch [{epoch + 1}/{num_epochs}], Train Loss: {epoch_train_loss:.4f}, "
              f"Train Loss Std: {train_loss_std:.4f}, Val Loss: {epoch_val_loss:.4f}, "
              f"Val Loss Std: {val_loss_std:.4f}")
        for i in range(4):
            print(f"Distance Mean Point {i+1}: {distance_means[i][-1]:.4f}, "
                  f"Distance Std Point {i+1}: {distance_stds[i][-1]:.4f}")
        ...
        # Optionally add example images or other visualizations
    ...
    if not os.path.isfile(filename):
        ...
        # Load the existing DataFrame and append new results
